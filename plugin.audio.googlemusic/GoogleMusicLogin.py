...
class GoogleMusicLogin():
    ...
    def checkCookie(self):
        ...
        if self.settings.getSetting('cookie-date') != None and len(self.settings.getSetting('cookie-date')) > 0:
            #print "TEST : "+self.settings.getSetting('cookie-date')+" "+str((datetime.now() - datetime(*time.strptime(self.settings.getSetting('cookie-date'), '%Y-%m-%d %H:%M:%S.%f')[0:6])).days)
    ...
    def initDevice(self):
        ...
        if not device_id:
            ...
            self.login()
            webclient.session._authtoken = self.settings.getSetting('authtoken-mobile')
            ...
            try:
                ...
                self.main.log('No device found, using default.')
